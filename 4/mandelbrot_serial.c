...
int main() {
    ...
    printf("Rendering SERIAL selesai dalam %.2f detik.\n", duration_serial);

    // Simpan gambar hasil serial
    int result1 = stbi_write_png("mandelbrot_serial.png", width, height, NUM_CHANNELS, pixels, width * NUM_CHANNELS);
    if (result1) {
        printf("Gambar 'mandelbrot_serial.png' telah disimpan.\n");
    } else {
        printf("GAGAL menyimpan gambar 'mandelbrot_serial.png'!\n");
    }
    ...
    printf("Waktu Eksekusi Serial: %.6f detik\n", duration_serial);
